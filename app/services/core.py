...
async def display_user_balance(user_tg_id: int):
    ...
    try:
        ...
        message_answer = message_answer % (msg_balance % round(balance, 4))
...
async def get_current_value_currency():
    ...
    currency, input_price, output_price = Decimal(data["currency"]), Decimal(data["input_price"]), Decimal(data["output_price"])
